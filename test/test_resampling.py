...
def test_sine_wave_file(synchronize_clocks):
    ...
    clock_values = np.asarray(
        [
            7.571999958599918e-06, -1.375600004394073e-05, -4.471499778446741e-06, -5.567000698647462e-06, -3.704500159074087e-06, -1.195950062538031e-05, -1.54390008901828e-05, -7.877500138420146e-06, -1.074450028681895e-05, -5.218000296736136e-06, -2.928049980255309e-05, -1.033099943015259e-05, -2.814500476233661e-06, -1.19775004350231e-05, -1.19775004350231e-05, -1.19775004350231e-05, -1.19775004350231e-05, -1.19775004350231e-05, -1.19775004350231e-05, -1.19775004350231e-05, -1.19775004350231e-05, -1.19775004350231e-05, -1.19775004350231e-05, -1.19775004350231e-05 ,
        ]
    )
    ...
    assert streams[i]["info"]["nominal_srate"][0] == '512.000000000000000'
    assert streams[i]["info"]["channel_format"][0] == "float32"
    assert streams[i]["info"]["created_at"][0] == "15440.8167746200"
    desc = streams[i]["info"]["desc"][0]
    assert isinstance(desc, dict), f"Stream {i} desc not a dict {type(desc)}"
    ...
    assert streams[i]["info"]["effective_srate"] == 512.0000186214386
    ...
    # Footer
    assert streams[i]["footer"]["info"]["first_timestamp"][0] == "15468.02782607"
    assert streams[i]["footer"]["info"]["last_timestamp"][0] == "15527.03004352"
    assert streams[i]["footer"]["info"]["sample_count"][0] == "30722"
    first_clock_offset = streams[i]["footer"]["info"]["clock_offsets"][0]["offset"][0]
    assert first_clock_offset["time"][0] == "15468.02782607"
    first_clock_offest_value = format(float(first_clock_offset["value"][0]), '.8f')
    assert  first_clock_offest_value == "15527.03004352"
